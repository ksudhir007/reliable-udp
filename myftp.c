...
char* strings_state[] = {"Congestion States","Slow Start", "Congestion Avoidance"};
...
void sendFile(int sockfd, char* fileName, struct sockaddr* cliAddr, socklen_t cliAddrLen, int cliWindow, int servWindow)
{
	...
	int expectedACK =0;
	int isLast = 0;
	int fileLastSequence=-1;

	int congestionSeqProb = 0;	//It checks the following case packets 1,2,3,4,5 sent -- Packet 2 drops -- retransmit packet 2
					//set the variable to 1. means 
...
check_window:
	...
	printf("Current Window Size = %d, ssthreshold = %d, state = %s\n", myCongestion.currWindowSize, myCongestion.ssthresh, strings_state[myCongestion.state]);
	firstPacket = 0;
	...
	if(getLength(head) > 0 && (getLength(head) < myCongestion.currWindowSize) &&  recvCliWindow > 0 && tempWindow > 0)
	{
		printf("Some packets already in sending buffer\n");
		tempWindow = myCongestion.currWindowSize;
		packet = head;
		my_rtt_newpack(&rttinfo);
...
wait_ACK:
	...
	if(FD_ISSET(sockfd, &rset))
	{
		...
		if(recvhdr.isACK ==1)
		{
			...
			if(isLast == 1)
			{
				if(receivedACK == fileLastSequence)
				{
