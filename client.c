...
send_filename:	
	...
	if(FD_ISSET(sockfd, &rset))
	{
		...
		if(random_dropper >= c_arguments->probability)
		{		
		...
		{
			printf("Dropping port number received from server since generated random probability %f < %f.\n", random_dropper, c_arguments->probability);
			goto send_filename;
		}
	}
	else
...
send_ACK_for_port:
	...
	{
		printf("Dropping window size packet. Not sending it to the server.\n");
...
int main(int argc, char **argv)
{
	...
	*/
	...
	if(flag_same == 1)
	{
		...
		{
			locAddr = ((struct sockaddr_in *)ifiClientNew->ifi_addr);
			cliaddr.sin_addr = locAddr->sin_addr;
