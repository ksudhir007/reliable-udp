	...
	while(1)
	{
		...
		while(temp->next != NULL)
		{
			if (FD_ISSET(temp->sockfd, &temp_set)) 
			{
				...
				if(0 == already_exists)
				{
					printf("This is new client, forking a child process to handle it.\n");
					pid = fork();
...
//			sendFile(connection_sock_fd, fileName, clientWindowSize,s_arguments.window_size) ;
			printf("Handshake completed successfully, starting data transmission.\n");
			sendFile(connection_sock_fd, fileName, (struct sockaddr*)&newCliSock, newCliSocklen, clientWindowSize, s_arguments.window_size);
				...
				{
					printf("Received data from same client, not forked this time. The child process is taking care of it!\n");
				}
			}
			temp = temp->next;
			interface_counter++;
